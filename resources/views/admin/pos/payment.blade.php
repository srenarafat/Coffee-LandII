            ...
            <div class="d-flex flex-wrap justify-content-between align-items-start mb-4">
                ...
                <div style="flex: 1 1 55%;">
                    ...
                    <div class="mb-2">
                        ...
                        {{-- CHANGED: use text+decimal so trailing dot is allowed while typing --}}
                        <input
  type="text"
  name="cash_usd"
  id="cashInputUsd"
                ...
                <div style="flex: 1 1 40%; margin-left:15px;" class="text-center">
                    <div class="d-grid" style="grid-template-columns: repeat(3, 1fr); gap: 15px; min-height: 300px;">
                        @foreach ([7,8,9,4,5,6,1,2,3,0] as $num)
                            <button type="button" class="btn number-button" onclick="appendNumber({{ $num }})">{{ $num }}</button>
                        @endforeach

                        {{-- Dot (.) --}}
                        <button type="button" class="btn number-button" style="font-size: 22px;" onclick="appendNumber('.')">.</button>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.appendNumber = function(num) {
        ...
        selectedInput.focus(); // keep box-shadow visible
        ...
        // Skip clamping while typing a trailing dot (e.g., "2.")
    ...
    // Only digits and a single dot while typing
...
cashInputUsd.addEventListener('input', () => {
  ...
  // trigger your existing change calc
  cashInputUsd.dispatchEvent(new Event('input', { bubbles: true }));
});
